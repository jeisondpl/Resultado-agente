package ec.otecel.digercicservice.util;
...
import ec.otecel.digercicservice.constants.MsConstants;
public abstract class BasicOperationService<R, P> {
	...
	public R run() throws ComponentException {
		...
		try {
			...
			throw createMessageFaultDTO(error);
		} catch (Exception e) {
	...
	private ComponentException createMessageFaultDTO(ComponentException request) {
		ComponentException componentException = request;
		if(componentException.getCauseMsg()==null){
			componentException.setCauseMsg("");
			return componentException;
		}
		if(!componentException.getErrorCode().getCode().equals(MsConstants.CODE_ERROR_DATA) && !componentException.getCauseMsg().equals(MsConstants.DATA_ERROR_LEGACY)) {
			componentException = new ComponentException(ErrorCodeType.ERROR_INESPERADO, MsConstants.INTERNAL_SERVER_ERROR,
					true, null,
					ErrorUtil.createErrorLocation(MsConstants.INTERNAL_SERVER_ERROR,
